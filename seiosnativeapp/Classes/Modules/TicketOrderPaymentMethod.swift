            ...
            post(parameter, url: path, method: "GET") { (succeeded, msg) -> () in
                DispatchQueue.main.async(execute: {
                    ...
                    if msg
                    {
                        ...
                        {
                            self.formDic = dic
//                            self.chequeForm = dic["chequeForm"] as! NSArray
    ...
    @objc func paymentOptionValueChanged(_ cell: FXFormFieldCellProtocol){
        let form = cell.field.form as! CreateNewForm
        Form.removeAll(keepingCapacity: false)
        Form = tempFormArray
        
        if let method = form.valuesByKey["method"] as? String {
            if method == "By Cheque" {
                if let chequeArray = self.formDic["chequeForm"] as? NSArray {
                    for i in 0 ..< (chequeArray.count) where i < chequeArray.count
                    {
                        Form.insert(chequeArray[i] as AnyObject, at: i + 1)
                    }
                }
            }
        }
